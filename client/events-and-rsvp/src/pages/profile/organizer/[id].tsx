...
import { useGetOrganizerProfileQuery, useUpdateOrganizerMutation } from '@/store/userApi';
...
const Profile = ({ id }: MyProfileProps) => {
    ...
    const { data: organizerData, refetch } = useGetOrganizerProfileQuery(id)
...
setPassword(organizerData?.password)
setPhone(organizerData?.phone)
setFullName(organizerData?.name)
    },[organizerData])
    ...
    const handleUpdateProfile = (e: any) => {
        e.preventDefault();
        // alert("updated")
        updateOrganizer({ name: fullName, password, phone, id }).unwrap()
        .then((res) => {
